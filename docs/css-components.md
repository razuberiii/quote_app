...
Chat-derived quote preparation stays inside the Smart Intake canvas. `.draft-preparation` is a compact readiness summary, not a dashboard card, and `.catalog-candidate.is-selected` identifies the seller's working option with a rule instead of a filled tile. Candidate rows may expose Catalog price, currency, MOQ, and specification warnings, but selecting one must keep every populated value editable and must never disguise an unpriced or currency-mismatched option as ready. All extracted specification keys remain visible; the review must not collapse them to a fixed product-specific shortlist.

The Inquiry review prioritizes the seller's immediate quote decision. `.draft-preparation` links directly to customer/term and product confirmation; the full readiness checklist uses the collapsed `.review-guidance--compact` disclosure, and chat history plus the follow-up composer use the collapsed `.inquiry-evidence-drawer`. These disclosures preserve evidence and secondary actions without placing them before the editable quote fields. Their summaries must state what is inside and may not use icon-only controls.
When the minimum draft requirements are present, `.draft-preparation__actions` exposes the form-associated Create draft action before the long review form. The form footer remains static; it must not cover editable fields with a viewport-sticky action bar.

Smart Intake review is an open editorial canvas. `.review-guidance` may use dividing rules but not a filled page-width slab; `.inquiry-source-panel` is a sticky evidence column with one red source rule rather than a rounded card. `.extracted-field`, `.inquiry-product-card`, `.catalog-match`, and Catalog candidate editors use section rules and underlined controls instead of nested filled boxes. Company profile review follows the same hierarchy through `.company-profile-review*`.
Legacy theme adapters must not include `.inquiry-source-panel` or `.inquiry-results-panel`, and global input theming must remain overridable without `!important`; otherwise Smart Intake cannot preserve its open canvas and underlined controls in dark mode.

Inquiry creation has one user-visible path: paste customer messages or upload the source file, then review the extracted quote information. AI extraction and deterministic fallback are internal behavior and must not appear as competing creation methods. The composer and next-step rail are separated by a single rule; neither may introduce a page-sized enclosing card or hard-coded theme surface.
...
Catalog review uses `.import-coverage*` for the page/Sheet processing ledger and `.candidate-*` for human decisions. Failed or unrecognized ranges remain visible above candidates. Account security overlays use the shared `.neo-modal` panel, field, state, and action families; inline light modal styling is prohibited. A security widget modal must always expose a visible close action and a timed load-failure state with retry; an empty modal or background-click-only escape is prohibited.
...
Authenticated pages use one open application canvas. The layout-level `main` is transparent; page shells provide width and rhythm but never a full-page opaque slab. Only functional regions such as tables, editors, dialogs and immutable documents may own an opaque surface. Layout notices are compact fixed status messages rather than full-width content bars.
...
The `.studio-inline-title` textarea grows with its content and may break long customer or proposal names at any character boundary; fixed-row clipping is not allowed inside the three-column editor.
Below 760px, `.quote-workspace .studio-structure.quote-requirements` becomes normal-flow content with intrinsic height. Desktop sticky-rail height must never leave a viewport-sized blank gap before the editable document on mobile. The mobile toolbar is an explicit two-column grid; it hides its secondary menu, live save label, and long title so the single preview action cannot collapse into vertical text. `.studio-product__top` keeps the mode control below the product identity instead of competing for its text width; `.studio-product__name` owns the full content column as a content-sized multiline editor so product names wrap instead of clipping or becoming a one-character column.
On mobile, Quote Studio is a sequence of separated paper sections rather than one continuous document strip. Cover, products, pricing, terms, and save footer each receive their own boundary and compact rhythm; the pricing section remains light in the editor so one dark block does not dominate the page. Quote document headings use the body typeface at medium-semibold weight with relaxed line height. Condensed display type and sub-1.0 heading line heights are prohibited for long quote and product names.
Mobile quote headings must remain within their section at arbitrary content length using bounded inline size and safe wrapping. The toolbar return action uses the compact `.studio-back` control with an arrow and short label; unframed low-contrast navigation text is prohibited.
...
Each Quote Studio readiness issue links to its owning section: customer/currency/validity to cover, item issues to products, freight/total to pricing, and payment/Incoterm to terms. A generic link that sends every blocker to pricing is prohibited.
Quote Studio performs a real debounced form save after edits. A Turbo navigation attempted while the form is dirty is deferred until that save succeeds; status text must never claim automatic saving when only browser-side form state has changed.
After a successful autosave, Quote Studio replaces the stale readiness panel and primary preview/publish action from the server-rendered response. “Saved” may not coexist with a stale “save to refresh checks” message.
Quote Studio form controls are owned canonically by the `--studio-*` layer in `workspace.css`: 40px minimum height on desktop, 44px on mobile, themed neutral fill, visible blue focus ring, consistent padding, and readable body typography. `rubusoo.css` must not carry a competing Studio input block. Dates, selects, numbers, and text fields may not fall back to compact legacy gray controls. Inline document titles remain borderless but gain a clear underline on focus.
On desktop, Quote Studio reserves compact fixed rails (210px outline and 240px summary) so the editable document receives the majority of the viewport. Full-width metadata rows such as buyer language stack their label above a two-column control/help row; helper copy must never collapse into a one-character column.
...
After publication, the seller success preview renders the newly published immutable Revision and its snapshot. It must not display the next working revision number, a generic Preview version label, or the internal-preview warning. Quote Studio exposes one context-aware preview/publish action rather than two links to the same preview route.
...
Pending email verification uses the single-task `.email-verification*` canvas: visible product identity, one primary code action, and a ruled secondary resend row. Bootstrap grid/alert shells, nested cards, inline presentation styles, and low-contrast account recovery links are prohibited.

Buyer Room uses `buyer-storefront` as a branded commercial microsite: editorial cover, visual product stories, working selections, plan comparison, commercial terms, and a sticky decision summary. In light mode the desktop summary remains a light paper-adjacent rail separated by a rule; a page-height dark block is prohibited. Below 980px it becomes a compact bottom decision bar. Dialogs must preserve the selected plan, quantities, and accessories without mutating the formal Revision.
Buyer-facing quote headings use the readable body typeface at a restrained semibold weight and a minimum 1.05 line height. Mobile sections use compact but distinct vertical groups; oversized condensed black headings that visually merge across lines are prohibited.
Seller-only Buyer Room preview status uses a high-contrast rose warning block when publishing is blocked. Readiness text may not consume generic application theme tokens on the light buyer surface, and every mobile heading must safely wrap within the viewport.
The mobile Buyer Room decision bar stays on the same light surface as the document with a top rule and elevation. A black viewport-wide strip is prohibited; primary and secondary actions must retain explicit contrast on the light bar.
...
Quote creation has two entry paths: customer-content preparation is recommended for an empty account, while a blank draft remains available for sellers who already know what to enter. `.quote-start*` selects or creates the customer on an open editorial canvas, then opens Quote Studio with one unpriced placeholder item. Draft saving remains available immediately; publication remains blocked until required commercial data is complete.

For an empty account, first-quote activation is task-led rather than module-led. `.first-quote-launch` and `.first-quote-choice` recommend connecting chat synchronization first, explicitly naming WhatsApp Web and Alibaba International Message Center. Paste/upload and blank creation remain progressively disclosed fallbacks through `.manual-quote-path`; they must not compete visually with plugin connection. `.inquiry-import--focused` is the fallback source composer and links back to chat synchronization; processing details belong in `.inquiry-import__details`.

Chat synchronization setup keeps installation, pairing, and in-platform binding as the three primary steps. `.chat-sync-conversations` is the persistent return path for previously bound conversations and links to their Inquiry review; opaque platform conversation identifiers remain implementation metadata rather than user-facing navigation labels.

The seller-visible work unit before quote creation is an Inquiry. `.inquiry-task-index` and `.inquiry-task-list` provide the persistent resume surface across plugin and fallback imports. A repeated binding of the same platform conversation resumes its current Inquiry; after a terminal quote, the first genuinely new buyer message with explicit quote/order intent starts a new Inquiry and the row explains that task boundary through `.inquiry-task-list__reason`. Replayed and ordinary post-sale messages never create a second task.

Chat integration setup uses `.chat-sync-setup*` as an open application canvas with three ruled setup sections and a flat token ledger. The embedded website panel itself is isolated in Shadow DOM and must not reuse or inject Rubusoo application selectors into the host chat website.
Pairing codes use `.chat-sync-code__actions` to keep copy and expiry feedback adjacent to the code. A pairing flow must not require manual transcription or hide its remaining lifetime.
Userscript installation opens separately from the setup task and keeps `.chat-sync-install-note` visible beside the install action. The note must explain how to return to pairing and what to check when no installer appears.
...
- `.commercial-pdf__custom` renders snapshot-frozen company-defined fields in a compact two-column block; blank values are omitted and long values wrap.
...
- Catalog candidate actions may stay sticky on desktop, but become a static footer below `760px`; they must not cover editable candidate fields on mobile.
- Catalog processing uses quiet JSON polling and replaces the page only when processing finishes; full-page timed refreshes are prohibited. Empty recognition results retain the source and expose a clear retry action.
- Quote Studio save feedback remains visible on mobile and includes the server-confirmed save time after a successful draft save.
- Quote Studio toolbar, requirement rail and action rail inherit `--studio-text` and `--studio-muted`; hard-coded dark-theme text colors must not be reintroduced. The cover metadata reserves a usable final column for the currency control on desktop.
- Quote requirement-state rows use short localized state and field labels in a fixed status column; raw extraction keys and long English state names must not be exposed in the narrow rail.
