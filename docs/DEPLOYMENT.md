...
Current production deployment: `https://next.rubusoo.com`. The immutable image tag matches the deployment commit reported by `git rev-parse --short HEAD`. Nginx configuration is `/etc/nginx/sites-available/next.rubusoo.com`; the pre-change backup is `/root/nginx-conf-backups/20260716T021026Z`. The Let's Encrypt certificate expires 2026-10-14 and has automatic renewal configured.
