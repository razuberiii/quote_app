...
`InquiryAiExtractor` calls an OpenAI-compatible Chat Completions endpoint and returns a constrained JSON structure. Provider settings are supplied only through `OPENAI_API_KEY`, `OPENAI_BASE_URL`, and `OPENAI_MODEL`; secrets must never be committed. Extraction never supplies prices or freight amounts and falls back to manual review when the provider is unavailable.
